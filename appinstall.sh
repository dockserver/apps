...
function progressfail() {
  ...
  apts='tar curl wget pigz rsync'
  for apts in ${apts}; do 
    command -v ${apts} >/dev/null 2>&1 && echo >&2 "check : ${apts} is installed" || { echo >&2 "I require ${apts} but it's not installed. Now we install ${apts}."; $(which apt) install -y ${apts}; }
  done
  unset apts
